...
### v0.3.0（当前）
- **🛡️ 稳赢-回本模式**（默认开）：把「参与市场后过度权重赔率」问题正面解决。
  - **贝叶斯收缩**：LLM 概率与市场共识按 `confidence` 加权混合，减少 LLM 高估小概率事件时被误推荐；越自信越信 LLM，越不自信越靠市场。
  - **效应门槛**：赔率越高，EV 要求越严（`odds=2` 不变，`odds=5` 约 2.32×，`odds=10` 约 3.32×）；避免"名义正 EV 实则赌运气"。
  - **三档分级**：🟢 稳赢推荐（混合胜率 ≥ 55%）／⚠️ 冷门博（小仓位、二次确认、余额上限 5%、Kelly 打 3 折）／⬜ 建议跳过。
- **📊 面板重做**：每个选项行展示 LLM/市场/混合三列概率并排；冷门推荐用橙色明显标识和额外 confirm 弹框。
- **⚙️ 设置面板新增高级选项**：稳赢模式开关、冷门开关、`pFinalSafe`（0.55）/`longshotEvMul`（2.0）/`longshotStakeMul`（0.3）/`longshotBalanceCap`（0.05）四个可调参数。
